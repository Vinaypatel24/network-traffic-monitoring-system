...
public class CaptureSimulationService {
    ...
    private static final List<String> COMMON_EXT_IPS = List.of(
            "8.8.8.8", "1.1.1.1", "142.250.190.46", "151.101.1.140", "13.107.42.16", "20.190.159.2", "192.168.1.105"
    );
    ...
    public void startSimulation(CaptureSession session) {
        ...
        while (isRunning.get()) {
            try {
                ...
                // Occasional threat trigger injection (e.g. port scan simulation or spike every ~30 ticks)
                boolean injectScan = (tickCounter % 35 == 0);

                if (injectScan) {
                    // Simulate port scanning from an attacker IP
                    String attackerIp = "198.51.100.77";
                    String targetIp = "192.168.1.10";
                    for (int p = 1000; p < 1030; p++) {
